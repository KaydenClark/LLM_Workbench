...
5. Follow the workspace branch policy. By default, publish task checkpoints and
   completed work to a task branch, promote verified work only to `integration`,
   and leave `integration` to `main` for the owner.
6. Always commit and push after every completed ticket and before yielding incomplete
   work. A local-only scaffold is not a completed Genesis handoff.
