...
| `make-it-so` | Confirm approvals and execute — with or without a grilling. A thin owner-only delegator: invokes `/to-docs`, `/to-spec`, `/to-tickets`, `/save` for the plan, then `/tdd` -> `/code-review` -> `/save` per ticket, and deletes the promoted notepad; invoked explicitly, never from the phrase. | Native | Active |
...
| `lexicon` | Use the project's `LEXICON.md` as the map: load it whole, speak its terms exactly, follow each term to the doc that owns it, and keep its open questions open. | Reference | Active |
