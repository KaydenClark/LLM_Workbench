...
| `make-it-so` | Kayden's "approved — build it and save it" command, with or without a grilling. One invocation composes to-docs, to-spec, to-tickets, schedule, and implement over the settled decisions (matching grilling notepad, else the current conversation), then commits and pushes everything so progress is saved; invoked explicitly, never from the phrase. | Native | Active |
