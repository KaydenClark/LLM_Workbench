...
| **SPEC and TASK** | The Spec is the durable capability owner. Its Tasks divide delivery into temporary slices, each held in its own `TASK.md`. Neither a dashboard nor a local task list replaces their accepted state. |
...
| **Spec** | A stable capability record containing scoped intent, requirements, decisions, implementation slices, acceptance, verification, evidence, and completion. | It combines the useful product and engineering roles often split between a PRD and technical spec. A Spec has many Tasks and many Chats: one per Task, plus the Chats that direct and review it. |
| **Task** | The execution slice: one bounded, executable thin vertical slice of a Spec, with its own state and blocking relationships, held in its own `TASK.md`. One Task is worked in one Chat. | It is execution structure, not durable capability history. A Task spanning a few Chats is tolerated but not intended; the fix is a smaller Task, still as vertical a slice as possible. Small, localized work may be a Task directly under the Blueprint with no Spec where the room's controls and tooling support it. Work expected to need more than one context unit is a Spec with Tasks, not a Task. |
...
| **Director** | The role above the Dispatchers for a version of work: it starts a Dispatcher for each Spec, coordinates between them, approves each assembled Spec in a separate context before it combines into the declared integration branch, and escalates blockers to the owner. | Defined by responsibility, not by branch. The owner is the human above the Director, not the Director; Human QA stays the owner's own act, normally after the Director has approved the version's Specs or escalated their blockers, at times the owner chooses, with approval recorded per Spec. A Director never executes a Task. It is not a stance or an authority grant, and it never bypasses the `AGENTS.md` integration gate. A shipped `carry` skill may not describe these roles yet. |
| **Dispatcher** | The role that carries one Spec, or a Task that advances the Blueprint directly: it plans the work, sends Workers to as many non-conflicting Tasks as can run in parallel, reads their reports, chooses the next step, merges for containment, and does the whole-Spec Verify/QA itself or dispatches it while owning the result. | A separate role from the Director, not an alias for it. It keeps the single durable writer for shared Spec state that `AGENTS.md` names. Merging a Task is containment, not a review, and a Dispatcher cannot give the Director's approval of work it carried. |
| **Worker** | The role that carries out one Task in one attempt: it does the step, self-checks that its claims are valid and backed by proof, updates the card and the Task's records as it goes, hands back and ends. | A Task has no review or approval gate; the Worker's self-check is part of doing it. A missed step is not reopened: its `TASK.md` stays the record, its card returns to In progress and a new Task fixes it. A Worker that implemented a Task in a candidate cannot approve that Spec. |
