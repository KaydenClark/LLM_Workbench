...
| **Spec** | A stable capability record containing scoped intent, requirements, decisions, implementation slices, acceptance, verification, evidence, and completion. | It combines the useful product and engineering roles often split between a PRD and technical spec. A Spec has many Tasks and is worked across many Chats, one Chat per Task. |
| **Task** | The execution slice: one bounded, executable thin vertical slice of a Spec, with its own state and blocking relationships, held in its own `TASK.md`. Small, localized work may be a Task directly under the Blueprint with no Spec. One Task is worked in one Chat. | It is execution structure, not durable capability history. A Task that needs several Chats was sliced too large: cut it smaller, still as a vertical slice. Work expected to need more than one context unit is a Spec with Tasks, not a Task. |
| **Chat** | One owner-visible working context in a host, in which participants have a Conversation. A Chat works one Task: it completes that Task, cleans up and ends. | A Chat is not a Task, assignment, durable record, or authority boundary, and it never works several Tasks. Several related Chats may belong to one Thread. |
...
| **Director** | The way of working a Spec as a whole: watch it as it progresses, write a handoff and open a new Chat for each Task as it becomes unblocked, and approve or route review of the Tasks that need it. Dispatcher is an accepted alias. | A Director never executes a Task itself; it watches and approves. Carrying a Spec to completion means directing it, because one Chat carries only one Task. It is not a stance or an authority grant, and it keeps the single durable writer for shared Spec state that `AGENTS.md` names. |
