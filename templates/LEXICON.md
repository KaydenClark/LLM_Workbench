...
| **Director** | The way of working a Spec as a whole: watch it as it progresses, write a handoff and open a new Chat for each Task as it becomes unblocked, and approve or route review of the Tasks that need it. Dispatcher is an accepted alias. | A Director never executes a Task itself; it watches and approves. Carrying a Spec to completion means directing it, because one Chat carries only one Task; a shipped `carry` skill may not say this yet. It is not a stance or an authority grant, and it keeps the single durable writer for shared Spec state that `AGENTS.md` names. |
