...
function validatePlan(plan, source, release, snapshot) {
  ...
  exactKeys(cap, ['id', 'title', 'derived_from', 'outcome', 'acceptance', 'task'], ['id', 'title', 'derived_from', 'outcome', 'acceptance', 'task'], 'capability');
  ...
  exactKeys(cap.task, ['id', 'slice'], ['id', 'slice'], 'capability.task');
  if (!TASK_ID.test(oneLine(cap.task.id, 'capability.task.id'))) fail('invalid-plan', 'capability.task.id must be a stable TK- identifier');
  const task = { id: cap.task.id, slice: oneLine(cap.task.slice, 'capability.task.slice') };
  return { project, drafts, memory: { file: memory.relative, sha256: memory.sha256, bytes: memory.bytes }, selectedQuestions, activeAdrIds, capability: { id: cap.id, title, outcome, derived_from: [...cap.derived_from], acceptance, task } };
}
