...
| [S-042](workbench/specs/S-042-installed-state-repair/SPEC.md) | Acceptance / owner gate | unassigned | none | All three slices landed red/green: a seed record and `stale-seed`, explicit `adr`/`wiki` normalize modes, and `unverified-provenance` with a verified `record-source` command. This repository's own manifest provenance is now reported and was deliberately left unrepaired. | Independent review of the exact candidate, then merge into `integration`. |
