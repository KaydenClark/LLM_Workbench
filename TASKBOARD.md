...
- **Shipping now:** Lifecycle hardening and copy-readiness (T-005-T-009): a blank product README, mechanical scope enforcement, stale-claim/archival/versioning rules, and the downstream feedback loop.
- **Health:** green - all self-tests pass; root dogfood docs score full rubric (113/113); blank templates score 98/113.
- **Decision needed:** none open. D-001 resolved 2026-07-01 (extract + close PR #4).
- **Blocked on:** nothing; the T-002/T-003/T-004 stack merged to `main` (PR #9); PR #4 closed.
- **Next milestone:** copy-ready handoff - templates ready to seed a real downstream project.
...
| T-007 | 6 | Mechanical scope enforcement: optional `.claude/settings.json` template (deny secrets/build output; allowlist from filled edit scope), generated during bootstrap. Carries the extracted D-001 idea: declarative scope keys (`writable_roots`, `forbidden_paths`, `requires_review_for`) as the source for the deny/allow rules | prose scope is honor-system; hooks make it mechanical | `templates/` | manual check: settings file validates in Claude Code | README, GENESIS | agent | ready | 2026-07-01 |
