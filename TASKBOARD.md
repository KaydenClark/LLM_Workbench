...
| [S-044](workbench/specs/S-044-legacy-room-classification/SPEC.md) | Acceptance / owner gate | unassigned | none | Both slices are done: one `unreconciled-controls` refusal names every failing control, and `tools/workbench-classify.mjs` reports a lifecycle verdict with its evidence and writes nothing. | Independent integration review of `claude/s044-v3-1-2` before it merges into `integration`. |
<!-- hot-specs:end -->
