...
| [S-036](workbench/specs/S-036-v3-1-2-evidence-corrections/SPEC.md) | TK-001: Align the shipped Claude permission template, diagnostic, protocols, and tests with one supported `Edit`-rule model that reports restrictive uncertainty (ready) | codex-gpt-5 | none | Corrective scope reconciled against the referenced review, current integration source, Claude Code 2.1.212, and current Anthropic permission documentation. | Claim TK-001 and correct the Claude Code permission model through the template, diagnostic, prose, and tests. |
<!-- hot-specs:end -->
