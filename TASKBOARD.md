...
| [S-044](workbench/specs/S-044-legacy-room-classification/SPEC.md) | Acceptance / owner gate | unassigned | none | Both slices are done, and the classification rule has been repaired twice after separate-context review: the second round fixed the classes the first round had fixed only for their named instances - every lane the room will not open is now a room condition rather than an exit 1, and nothing under a non-ordinary support root is read, the managed tools lane included. | Independent integration review of `claude/s044-v3-1-2` before it merges into `integration`. |
<!-- hot-specs:end -->
