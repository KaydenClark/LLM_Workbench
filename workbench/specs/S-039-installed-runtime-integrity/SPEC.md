...
- `tools-receipt-drift` is emitted at exactly one site,
  `tools/workbench-tools.mjs:181`. `tools/workbench-tools.mjs` is not a member
...
- `tools-receipt-missing` is emitted from an installed tool, but only by
  `validateGenesisRuntime` (`workbench/tools/workbench-layout.mjs:499-521`),
...
- `RUNBOOK.md:581` lists both codes in the effect table's `all` row, so the published
...
- [ ] `RUNBOOK.md` describes the installed check and the three states; the
      effect table at `RUNBOOK.md:579-584` is true of observable behavior.
