...
**Blockers:** none
**Latest event:** Third review repair: the authoritative managed set moved into the lane a room installs, so a managed file deleted together with its receipt key is named instead of passing silently; the two coverage conditions carry distinct working remedies; and the three false or unfalsifiable claims are corrected in the spec, both runbooks, and the code comment.
...
- **A receipt rewritten to record tampered bytes is invisible to both paths.**
  ...
  recorded hash together. Closing it needs an authority the receipt cannot
