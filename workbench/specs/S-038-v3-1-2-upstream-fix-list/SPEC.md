...
> **Citation anchors.** pre=`09bfff7` post=`8dfd1a8`. A label before a citation names
...
| 2026-09-06 | TK-002 | Correcting the row above: it undercounts the HIGH findings it reports | The row says the three CHANGES REQUESTED reviews raised "two findings rated HIGH". Recounted from the four review reports: S-039 one HIGH (nine stale citations), S-040 one HIGH (the false host record), S-044 **two** HIGH (the exculpatory clause that is itself false, and the four-new-cases count that is wrong at every granularity), S-043 none. **Four, not two.** The undercount was written into the same commit that rewrote this section to withdraw a different undercount, and it ran in the same direction as every prior error here - toward flattering the gate by making its findings look smaller. The gate account prose is repaired; this row corrects the published one | Prose at the section "What the gate cost, honestly" now enumerates the four by spec; `benchmarks/RESULTS.md` carries the same correction as its own appended row | None. The count is now stated per spec so it can be checked without a convention |
| 2026-09-06 | TK-002 | Correcting the S-042 conditional in the row above, which contrasts a number with itself | That row ends "if it did not, the count is four deviations of six rather than four with one unevidenced" - the same figure on both sides of a contrast that exists to distinguish them. Four deviations are established: S-039, S-040, S-043, S-044. If S-042's approving review did not happen it is the **fifth of six**, and S-041 becomes the only slice in v3.1.2 with an evidenced approving review. The same off-by-one shipped in S-042 and S-045 in the same commit and is corrected in each. Found by the separate-context review of `49a6246`, which also found this branch's largest single error: S-045's row claiming all four originating specs had gained a gate-deviation row naming it, when S-040 had gained nothing - the spec the entire gate thread started from | S-040 now carries its retrospective row, its repaired header and a Remaining Limitation naming S-045 TK-005; S-045's stale three-ticket prose is repaired and re-rendered | None new. Every correction here is an appended row; no published row was edited |
...
defects three times, raising four findings rated HIGH between them - one in
S-039, one in S-040, two in S-044 - in candidates that a green suite and a clean
`doctor` had already passed.
