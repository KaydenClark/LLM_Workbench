...
**Blockers:** none
**Latest event:** TK-002 claimed by claude-lane-D.
...
| TK-002 | Register and surface the git-state findings | in-progress | none | Red test asserting `detached-head` and `untracked-controls` are registered `attention`/`none` and appear in `doctor` output for a fixture; green findings; proof that neither changes `doctor`'s exit code or `next`'s selection |
