...
- TK-003 depends on whichever source verification `init` carries when it is
  built. At `b3633e5` that is `invalid-invocation`; the unmerged S-036 branch
  replaces it with a stricter `invalid-source-identity`. Read the tool before
  writing the test rather than trusting either name from this spec.
...
- `RUNBOOK.md:579-584` effect table: the new codes and their effects.
