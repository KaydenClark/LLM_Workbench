...
- `unrecorded` no longer exists in the layout tool; `init` and `migrate` resolve
  the release checkout or refuse with `invalid-source-identity`
  (`workbench/tools/workbench-layout.mjs:340-360`), and
  `tools/test-workbench-layout.mjs:848` guards the placeholder's absence. But no
...
3. A registered diagnostic reports a manifest whose `provenance.source` is a
   ...
   same `invalid-source-identity` verification that guards `init`.
...
`invalid-source-identity` verification, and prove it refuses from a relocated
partial copy.
...
- `RUNBOOK.md:588` effect table: the new codes and their effects.
