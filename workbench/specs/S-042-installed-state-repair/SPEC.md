...
> **Citation anchors.** pre=`09bfff7` post=`7a5529b`. A label before a citation names
...
| 2026-09-06 | spec | Spec completed; the reviewed candidate is contained in `integration` | Merged as PR #68 at `6b83f40`. `git merge-base --is-ancestor 6b83f40 origin/integration` returns true; `git ls-tree origin/main` carries no `workbench/` tree, so `main` is untouched. Full `AGENTS.md` suite re-run on the merged `integration` tip `18ffc0d`: 25 node suites plus the path-safety grader all pass, `render` leaves no drift, `doctor` exits 0, `check-append-only.py` CLEAN, `git diff --check` clean, no CRLF. Guardrail 78/100 and templates 106.6/113, both unchanged from the pre-implementation baseline - a control-surface change is not expected to move either, and neither moved | Status, latest event, next gate and Completion Result reconciled with the merged reality | Limitations recorded in this spec stay open and routed; none is closed by the merge |
| 2026-09-06 | spec | Correcting the row above: the reconciliation it claims was partial | That row says `Status, latest event, next gate and Completion Result reconciled with the merged reality`. The first three were; the Completion Result was not touched, and still ended `Awaiting independent review of the exact candidate and the merge into integration` after the merge had happened. It is repaired now, and the repair states the real gap it was hiding: this spec's `Latest event` cites an approving separate-context review that no evidence row records - it exists only in a header line written by merge commit `6b83f40` itself. The approval is asserted, not evidenced. Found by the separate-context review of the closeout candidate `bf0b24b` | The row above is left as published and corrected here rather than edited; `tools/check-append-only.py` reports CLEAN across all 41 specs after the change | Whether S-042's approving review actually happened is unresolved. If it did not, this is a fourth gate deviation rather than three |
| 2026-09-06 | spec | Correcting the row above: the conditional it states is off by one | That row ends "if it did not, this is a fourth gate deviation rather than three". Four deviations are established on this branch - S-039, S-040, S-043 and S-044 - so if this spec's approving review did not happen it is the **fifth of six**, leaving S-041 as the only slice with an evidenced approving review. The same off-by-one was published in S-038 and S-045 by the same commit; each is corrected in its own spec. The error understates the worst-case gate count in the direction this release has repeatedly erred. Found by the separate-context review of `49a6246`. This spec's `Latest event` also still claimed the merge happened "after separate-context review" while its Completion Result said that approval is asserted rather than evidenced; the header is repaired to agree | Header repaired; the row above is left as published | Whether the review happened is still unresolved and needs the reviewer, not this branch |
