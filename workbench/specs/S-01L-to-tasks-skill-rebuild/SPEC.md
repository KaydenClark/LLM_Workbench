...
**Next gate:** Review this skill's existing behavior, then activate TK-01C for this skill only.
...
|---|---|---|---|---|
| TK-01C | Audit to-tasks, deliver the smallest supported source/documentation change and prove the routed article | ready | none | pending |
...
- [ ] Each Task is one bounded vertical slice with a named destination, dependency, stance and checkable proof.
- [ ] The Taskboard is a projection; no external tracker or standalone objective is invented.
- [ ] The named scenario is observed in a fresh or otherwise independent context: A large capability becomes independently executable slices and the next eligible Task is unambiguous.
- [ ] `workbench/wiki/skill-to-tasks.md` accurately distinguishes verified current behavior from remaining intended behavior, links the current source and governing owners, and is reachable from `workbench/wiki/MEMORY.md`.
...
Pending. Planning only; no to-tasks rebuild or behavioral acceptance is claimed.
