...
**Blockers:** none
**Latest event:** Review of `bfd84b4` confirmed the code fixes and found four stale descriptions of the tools-lane rule; all are aligned and focused green at `7e0f3c4`.
**Next gate:** Rerun the full release gate and obtain fresh independent approval of the fully aligned exact candidate.
...
| 2026-09-06 | TK-004 | Second immutable candidate review requested changes | Separate-context review of `ef22d38f04dadce3eea7f2bcfd69860e5067bac9` confirmed the first four blockers closed, then reproduced a dirty-template upgrade exiting partial after replacing an installed skill because the early combined preflight omitted `templates`. It also found root Runbook wording that still treated a covering deny as a tools-lane exception. | The review required the owning Runbook and implementation to state the same covering-ask rule. | Add template source to both upgrade preflight modes, preserve the changed installed skill and untouched project on failure, align the Runbook, and submit a new candidate. |
| 2026-09-06 | TK-004 | Template preflight correction green at `72ce55c` | Red: the dirty-runtime case blocked before mutation while the new dirty-template case returned partial after skill mutation. Green: workbench-upgrade 6/6 now exercises both consumed-source variants and proves installed skill, backup root, and project remain untouched; workbench-layout 32/32; diagnostics 10/10; workbench-dogfood pass; git diff check passed before commit. | Root Runbook now requires a covering tools-lane ask, keeps intersecting deny rules visible, and names absolute and home-relative Claude paths; the Claude README names the same path forms. | Full release gate, exact remote SHA, fresh independent review, PR merge, and `integration` containment. |
