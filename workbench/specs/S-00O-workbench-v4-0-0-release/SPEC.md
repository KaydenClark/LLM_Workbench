...
| 2026-09-17 | spec | S-00H TK-002, TK-005, TK-006 and TK-008 landed (PRs #102, #104, #105, #100); the managed runtime set `RUNTIME_TOOLS` now holds eighteen files (`task-packet.mjs`, `task-receipt.mjs` added) and `workbench/manifest.json` declares `contextUnit`, both release-surface facts for the v4.0.0 stamp; S-00P runs on live Task records; S-00H is gated on TT-Q10 for TK-003, TK-004 and TK-007, so S-00I and S-00J remain blocked | Separate-context reviews PASS on every landed tip; full suite 42/42 on each | Recorded here for the version bump and the self-drift check; RUNBOOK gained the `convert-tasks` line (PR #103) | Owner answer to TT-Q10 unblocks the rest of S-00H; fresh rooms declare no `contextUnit` (generator fix owed) |
| 2026-09-17 | spec | Correction to the row above: `RUNTIME_TOOLS` now holds nineteen files (`task-packet.mjs` and `task-receipt.mjs` added to the seventeen recorded after TK-001), not eighteen | Separate-context reviewer of PR #106 counted the array at integration 860fd18; the dispatcher re-counted from the exported list | This row corrects the count for the v4.0.0 stamp; the row above stays as first published | none |
