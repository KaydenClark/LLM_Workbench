...
- [ ] A greenfield target receives project identity, controls, managed lanes and recovery proof based on the actual prompt.
- [ ] An existing project routes to adoption; template room state is not inherited as the new project's own state.
- [ ] The named scenario is observed in a fresh or otherwise independent context: A clean target is generated and verified while an existing-code target is refused or rerouted.
- [ ] `workbench/wiki/skill-genesis.md` accurately distinguishes verified current behavior from remaining intended behavior, links the current source and governing owners, and is reachable from `workbench/wiki/MEMORY.md`.
...
Pending. Planning only; no genesis rebuild or behavioral acceptance is claimed.
