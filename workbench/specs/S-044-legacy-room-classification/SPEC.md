...
| 2026-09-07 | spec | **WITHDRAWN: "round two's wording was true when written".** S-045 TK-007 | The claim appears in row 375, which corrects the TK-002 Proof column and then adds, of round two's "also compares" phrasing, that it "was true when written". Row 377's retrospective review already found it false under both readings, and row 378 supplied the granularity row 377 left off. It is withdrawn here rather than argued again, because it is the one statement in this log that a reader can still take as standing: rows 377 and 378 correct the sentence around it without ever retracting the clause itself. **The four-new-cases count, restated with its granularity named**: row 375 says "two of this round's four new cases break it". Two counts are in play and neither is four. Across `57c483f`'s new `classify` cases there are **five**, not four; of those five, **two** carry no before/after snapshot at all (case granularity) and **three** contain at least one unbracketed `classify` of a room (invocation granularity). Re-measured here rather than copied from row 378, because repeating a record instead of checking it is the fault this whole thread of corrections traces back to: `git show 57c483f -- tools/test-workbench-layout.mjs` adds exactly five `test('classify ...` cases, and reading each case body out of `git show 57c483f:tools/test-workbench-layout.mjs` gives 2 and 3. Every number in this restatement carries the granularity it was measured at, which is the convention row 378 established and this row is the first to apply from the outset | This row is the record; the two rows it withdraws from are left as published, per the append-only rule | None for this claim. The underlying coverage gap is closed separately by S-045 TK-003 |
| 2026-09-07 | spec | The coverage gap this spec's Proof column described is closed; the correcting row S-045 TK-003 owes is this one | Row 375 recorded eleven of seventeen `classify` cases carrying a before/after snapshot, and row 377's reviewer re-measured at invocation granularity: **21 of 34** bracketed. All **34 of 34** are bracketed now in `tools/test-workbench-layout.mjs`, including the two invocations that refuse the project path rather than classifying a room, and the EPERM probe that runs the classifier in-process under a monkey-patched `fs` - the case the reviewer named as the reason to count by invocation. Proved by mutation rather than by counting alone: a classifier that writes one file into every room it is handed fails **11** tests against the pre-ticket file and **17** against this one, so the added brackets catch six writes that previously went unseen. The suite is 55/55 green unmutated | No mutable text in this spec changes: its Proof column already states eleven of seventeen, which was true of the tree it describes and stays true of it | None. The claim row 375 said would become true if the missing snapshots were added is now true, and by the stricter invocation reading rather than the case reading it was written at |
| 2026-09-07 | spec | **Correcting the mutation counts in the row above: they depend on the probe placement and its error handling, and were stated as if they depended on neither.** Raised across two separate-context reviews of S-045's candidate | The row above says a classifier that writes into every room it is handed ``fails **11** tests against the pre-ticket file and **17** against this one, so the added brackets catch six writes that previously went unseen``. Re-measured in a throwaway worktree, three shapes, both trees. **A** - after the `invalid-project` guard, so only rooms actually classified are written to: **11** pre-ticket, **16** after. **B** - before that guard, writing to `path.resolve(projectPath)`, left to throw: **12** and **17**; the sealed-parent case chmods its parent to `0o000`, the write raises EACCES, and `invalid-project` becomes `invalid-invocation`, which the pre-ticket file already fails on. **B-prime** - B with the write in `try {} catch {}`: **11** and **17**. **The delta is five under all three.** Six came from comparing B-prime's suppressed pre-ticket 11 with B's unsuppressed 17. The mechanism the row gives is correct and was verified: under B-prime the sixth newly-red test is the refused-symlink case, failing on the `roomSnapshot(target)` bracket added here, because the write follows the link into the target | This row is the record; the row above and the first correction of it are left as published, per the append-only rule | None for the coverage claim, which no shape moves: **21 of 34** `classify` invocations bracketed before, **34 of 34** now. The lesson is about the number - a mutation count is not reproducible without its injection point and its error handling, and this log now carries both |
