...
- `tools/workbench-adoption.mjs:63-78` iterates
  `workbench/tools/workbench-layout.mjs:20` `controls` (seven files) and
...
- `templates/ADOPTION.md:275-276` makes the same requirement a completion box.
- No command classifies a room. `grep -rn classify` over `tools/` and
  `workbench/tools/` returns only `tools/control-fidelity.mjs` line
  classification and `tools/feedback-automation.mjs` decision classification;
  neither reads a room's lifecycle.
...
|---|---|---|---|---|---|
| 2026-09-06 | spec | Spec captured from upstream UP-022 and UP-023 and re-verified at `b3633e5` | Read `workbench-adoption.mjs:63-78` (returns on the first failing control), `workbench-layout.mjs:20,226`, `templates/ADOPTION.md:218-222,275-276`; grepped `classify` across `tools/` and `workbench/tools/` and found no lifecycle classifier | Blueprint catalog regenerated by render | Both slices open; the first-control-only behavior sharpens UP-022 beyond the upstream statement |
