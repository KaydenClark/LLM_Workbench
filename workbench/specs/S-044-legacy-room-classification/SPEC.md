...
| 2026-09-07 | spec | **Correcting the mutation counts in the row above: they are shape-dependent, and were stated as if they were not.** Raised by the separate-context review of S-045's candidate | The row above says a classifier that writes into every room it is handed ``fails **11** tests against the pre-ticket file and **17** against this one, so the added brackets catch six writes that previously went unseen``. Both numbers reproduce, but only for one injection shape, and the row names none. Re-measured in a throwaway worktree at S-045's candidate `5180c71`, same probe, two placements. **After** the `invalid-project` guard, so only rooms the classifier actually classifies are written to: **11** pre-ticket, **16** after - five. **Before** that guard, writing to `path.resolve(projectPath)`: **11** and **17** - six, because the refused-symlink case then writes through the link into the target directory the new `roomSnapshot(target)` bracket watches. The reviewer independently measured five shapes and got a delta of five for every one writing only into classified rooms, with the pre-ticket 11 reproducing under all of them. The recorded pair is therefore real but is a property of probe placement, not of the change. What no shape moves, and what this spec actually needed: **21 of 34** `classify` invocations bracketed before, **34 of 34** now | This row is the record; the row above is left as published, per the append-only rule | None for the coverage claim. The lesson is about the number, not the work: a mutation count without its injection point is not reproducible, and this log now carries the injection point |
