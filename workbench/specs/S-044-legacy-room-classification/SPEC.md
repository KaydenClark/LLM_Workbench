...
| 2026-09-07 | spec | **Supplying the two conditions the mutation counts in the row above depend on. The counts themselves are right** | The row above says a classifier that writes into every room it is handed ``fails **11** tests against the pre-ticket file and **17** against this one, so the added brackets catch six writes that previously went unseen``. Measured in a throwaway worktree, three shapes, both trees. **A** - after the `invalid-project` guard, so only rooms actually classified are written to: **11** and **16**, delta five. **B** - before that guard, writing to `path.resolve(projectPath)`, left to throw: **12** and **17**, delta five. **B-prime** - B with the write in `try {} catch {}`: **11** and **17**, delta **six**. B-prime is the shape originally run, so the row above is correct as written; what it lacked was the two conditions that make it re-derivable - the injection point and the error handling. Six appears under B-prime alone because under A the refused symlink is rejected before any write, so the `roomSnapshot(target)` bracket added here never fires, and under B that test is already red pre-ticket when the sealed parent raises EACCES and `invalid-project` becomes `invalid-invocation`; only under B-prime is the bracket catch visible, and it is the sixth newly-red test | This row is the record; the row above is left as published, per the append-only rule | None. The coverage claim no shape moves - **21 of 34** `classify` invocations bracketed before, **34 of 34** now - and the count in the row above stands. The lesson is only that a mutation count is not reproducible without its injection point and its error handling, and this log now carries both |
