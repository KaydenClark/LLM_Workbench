...
- [ ] The skill resumes from the assigned Spec and Task, verifies live state, composes implementation and save only within the endpoint, and records routine coordination defects.
- [ ] A missing separate-context review or owner decision stays visible; carry never manufactures approval or a new task.
- [ ] The named scenario is observed in a fresh or otherwise independent context: An interrupted assigned Task reaches its reviewable result with evidence and a named remaining gate.
- [ ] `workbench/wiki/skill-carry.md` accurately distinguishes verified current behavior from remaining intended behavior, links the current source and governing owners, and is reachable from `workbench/wiki/MEMORY.md`.
...
Pending. Planning only; no carry rebuild or behavioral acceptance is claimed.
