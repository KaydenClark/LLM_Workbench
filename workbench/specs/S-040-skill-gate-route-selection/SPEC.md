...
| 2026-09-06 | spec | Correcting the range named in the row above; the row before it already said otherwise | That row opens "Reviewed `git diff ce3da23 627968e`, the exact unreviewed range". It is a superset, not the exact range. `git log -S` places the review-1 row in `29cabf5` and the review-2 row in `dbe67de`, so review 2 examined `29cabf5`; the exact unreviewed range is `29cabf5..627968e`, and `ce3da23..627968e` additionally contains `29cabf5`, which had been reviewed. The gate-deviation row directly above states this correctly, so the branch published two incompatible accounts of which commits went unreviewed in adjacent rows. The error is in the safe direction - the review examined more than the gap, so nothing it concluded is undermined, and an independent reviewer re-verified every substantive conclusion the row draws. Found by the separate-context review of `8dfd1a8`, which approved that candidate with this recorded rather than blocking | The row above is left as published and corrected here | None. S-045 TK-005 still owns the host-record repair the review actually found |
