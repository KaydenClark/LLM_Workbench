...
| TK-003 | Bracket every `classify` invocation that classifies a room with a before/after room comparison | done | TK-005 | All 34 of 34 `classify` invocations in `tools/test-workbench-layout.mjs` are bracketed by a before/after `roomSnapshot` comparison, up from 21. Measured with a script that walks each invocation and its argument rather than counting cases. The two invocations that refuse the project path rather than classifying a room are bracketed too, and so is the EPERM probe that runs the classifier in-process under a monkey-patched `fs`, which S-044 named as the reason to count this way. Proved by mutation, not by counting - and the count is meaningless without the probe's placement AND its error handling, both of which the first statement of it omitted. Three shapes, each measured on both trees. **A**, injected after the `invalid-project` guard so only rooms the classifier actually classifies are written to: **11** pre-ticket, **16** here. **B**, injected before that guard, writing to `path.resolve(projectPath)` and left to throw: **12** and **17** - the sealed-parent case turns `invalid-project` into `invalid-invocation`, which the pre-ticket file already fails on. **B-prime**, B with the write wrapped in `try {} catch {}`: **11** and **17**. The delta is **five** under all three; only B-prime's pair reads as six, and it does so because its pre-ticket number is the one the swallowed error suppresses. The sixth newly-red test under B-prime is the refused-symlink case, caught by the `roomSnapshot(target)` bracket this ticket added, because the write follows the link into the target. The claim that survives every shape is the coverage one: 21 of 34 invocations bracketed before, 34 of 34 now. 55/55 green unmutated. Full AGENTS.md suite 30/30 green at ef44e8e. |
...
| 2026-09-07 | TK-003 | **Correcting the mutation counts in this log's TK-003 rows: they depend on the probe's placement and on its error handling, and were stated as if they depended on neither.** Raised across two separate-context reviews | The TK-003 closing row above, and the matching row in S-044, say a writing classifier "fails **11** tests against the pre-ticket file and **17** against this one, so six writes that previously went unseen are now caught". Re-measured in a throwaway worktree, three shapes on both trees. **A** - after the `invalid-project` guard, so only rooms the classifier actually classifies are written to: **11** pre-ticket, **16** here. **B** - before that guard, writing to `path.resolve(projectPath)`, left to throw: **12** and **17**; the sealed-parent case chmods its parent to `0o000`, so the write raises EACCES and `invalid-project` becomes `invalid-invocation`, which the pre-ticket file already fails on. **B-prime** - B with the write wrapped in `try {} catch {}`: **11** and **17**. The recorded pair is B-prime's, and B-prime is what was originally run. **The delta is five under all three shapes.** Six is an artefact of comparing B-prime's suppressed pre-ticket 11 against B's unsuppressed 17, and the row above states six. The mechanism it gives is nonetheless correct and was verified: under B-prime the sixth newly-red test is the refused-symlink case, failing on the `roomSnapshot(target)` bracket TK-003 added, because the write follows the link into the target directory. What survives every shape is the coverage claim: **21 of 34** invocations bracketed before, **34 of 34** now | The mutable TK-003 ticket-table Proof cell carries all three shapes; the frozen rows are left as published and corrected here and in S-044 | This is the fifth checkable number in this run recorded without the condition that makes it checkable, after the `main` ancestry claim, the TK-006 mutation class, the TK-006 scope count, and the first attempt at this row - which named the placement but not the error handling, and so was corrected into a second unreproducible form. The defect is never the arithmetic. It is omitting the tree, the procedure, or the flag a number was measured under, which is what makes it unfalsifiable to the next reader |
