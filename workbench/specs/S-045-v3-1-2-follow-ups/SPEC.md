...
| TK-003 | Bracket every `classify` invocation that classifies a room with a before/after room comparison | done | TK-005 | All 34 of 34 `classify` invocations in `tools/test-workbench-layout.mjs` are bracketed by a before/after `roomSnapshot` comparison, up from 21. Measured with a script that walks each invocation and its argument rather than counting cases. The two invocations that refuse the project path rather than classifying a room are bracketed too, and so is the EPERM probe that runs the classifier in-process under a monkey-patched `fs`, which S-044 named as the reason to count this way. Proved by mutation, not by counting - and the number is only meaningful with the injection shape named, which the first statement of it omitted. Injected **after** the `invalid-project` guard, so only rooms the classifier actually classifies are written to, a writing classifier fails **11** tests against the pre-ticket file and **16** against this one: five writes that previously went unseen. Injected **before** that guard and writing to `path.resolve(projectPath)`, it fails **11** and **17**, because the refused-symlink case then writes through the link into the target directory that this ticket's new `roomSnapshot(target)` bracket watches. Both reproduce; the delta is five under the shape that writes only into classified rooms, six under the shape that also reaches a refused path. The robust claim is the coverage one: 21 of 34 invocations bracketed before, 34 of 34 now. 55/55 green unmutated. Full AGENTS.md suite 30/30 green at ef44e8e. |
...
| 2026-09-07 | TK-003 | **Correcting the mutation counts in this log's TK-003 rows: they are shape-dependent, and were stated as if they were not.** Raised by the separate-context review of `origin/integration..54ec37f` | The TK-003 closing row above, and the matching row in S-044, say a writing classifier "fails **11** tests against the pre-ticket file and **17** against this one, so six writes that previously went unseen are now caught". Both numbers reproduce, but only for one injection shape, and the rows name no shape. Re-measured in a throwaway worktree at `5180c71`, with the same probe injected two ways. **After** the `invalid-project` guard, so only rooms the classifier actually classifies are written to: **11** pre-ticket, **16** here - a delta of five. **Before** that guard, writing to `path.resolve(projectPath)`: **11** and **17** - a delta of six, because the refused-symlink case then writes through the link into the target directory that TK-003's new `roomSnapshot(target)` bracket watches. The reviewer measured five injection shapes and got 5 for every one that writes only into classified rooms; the pre-ticket 11 reproduces under all of them. So the recorded pair is real but is a property of where the probe was placed, not of the change. The robust claim, which no shape moves, is the coverage one: **21 of 34** invocations bracketed before, **34 of 34** now | The mutable TK-003 ticket-table Proof cell now names both shapes and both deltas; the two frozen rows are left as published and corrected here and in S-044 | This is the fourth time in this run a checkable number was recorded without the condition that makes it checkable - after the `main` ancestry claim, the TK-006 mutation class, and the TK-006 scope count. The common shape is not carelessness about arithmetic but omitting the tree or the procedure a number was measured under, which is exactly what makes a number unfalsifiable to the next reader |
