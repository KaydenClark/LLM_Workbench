...
| TK-003 | Bracket every `classify` invocation that classifies a room with a before/after room comparison | done | TK-005 | All 34 of 34 `classify` invocations in `tools/test-workbench-layout.mjs` are bracketed by a before/after `roomSnapshot` comparison, up from 21. Measured with a script that walks each invocation and its argument rather than counting cases. The two invocations that refuse the project path rather than classifying a room are bracketed too, and so is the EPERM probe that runs the classifier in-process under a monkey-patched `fs`, which S-044 named as the reason to count this way. Proved by mutation, not by counting - and the count needs two conditions stated, the probe's placement and whether it swallows its own write error, neither of which the first statement of it gave. Three shapes, each measured on both trees. **A**, after the `invalid-project` guard so only rooms actually classified are written to: **11** pre-ticket, **16** here, delta five. **B**, before that guard, writing to `path.resolve(projectPath)`, left to throw: **12** and **17**, delta five. **B-prime**, B with the write in `try {} catch {}`: **11** and **17**, delta **six**. B-prime is the shape originally run, so the recorded pair and the six derived from it were both correct; only the conditions were missing. Six appears under B-prime alone for a reason that is itself the point: under A the refused symlink is rejected before any write, so the `roomSnapshot(target)` bracket this ticket added never fires; under B that same test is already red pre-ticket because the EACCES turns `invalid-project` into `invalid-invocation`, masking the catch; only under B-prime is the bracket's catch visible, and it is the sixth newly-red test. The claim no shape moves is the coverage one: 21 of 34 invocations bracketed before, 34 of 34 now. 55/55 green unmutated. Full AGENTS.md suite 30/30 green at ef44e8e. |
...
| 2026-09-07 | TK-003 | **Supplying the two conditions the TK-003 mutation counts depend on. The counts themselves were right; two earlier attempts at this row got them wrong in opposite directions** | The TK-003 closing row above says a writing classifier "fails **11** tests against the pre-ticket file and **17** against this one, so six writes that previously went unseen are now caught". Measured in a throwaway worktree, three shapes on both trees, each mutation committed because a dirty tree trips `invalid-source-identity`. **A** - after the `invalid-project` guard, so only rooms actually classified are written to: **11** and **16**, delta five. **B** - before that guard, writing to `path.resolve(projectPath)`, left to throw: **12** and **17**, delta five. **B-prime** - B with the write wrapped in `try {} catch {}`: **11** and **17**, delta **six**. B-prime is the shape that was originally run, so **the row above is correct as written**; what it lacked was the two conditions that let a reader re-derive it - the injection point and the error handling. Six appears under B-prime alone, and why is the substance rather than a footnote: under A the refused symlink is rejected before any write, so the `roomSnapshot(target)` bracket this ticket added never fires; under B that same test is already red pre-ticket, because the EACCES from the sealed parent turns `invalid-project` into `invalid-invocation`; only under B-prime is the bracket's catch visible, and it is the sixth newly-red test. The claim no shape moves: **21 of 34** invocations bracketed before, **34 of 34** now | The mutable TK-003 ticket-table Proof cell carries all three shapes and both conditions; the frozen rows are left as published and corrected here and in S-044 | **Two prior attempts at this correction were themselves wrong, in opposite directions, and both are left as published.** The first named the injection point but not the error handling, so its pair was still unreproducible. The second went further and asserted "the delta is five under all three shapes" while printing 11 and 17 four sentences earlier - withdrawing a true claim in favour of a false one, and contradicting itself by arithmetic anyone could check. Recorded because the pattern is the finding: five times in this run a number reached a record without the tree, procedure or flag it was measured under, and the sixth time over-correction destroyed a number that had been right all along. The reviews caught every one; the writing caught none |
