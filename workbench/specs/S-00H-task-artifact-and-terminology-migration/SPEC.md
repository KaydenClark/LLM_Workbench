...
blocked on this Spec reaching `complete`, because folder lifecycle cannot
reach a table row and the Spec QA gate reads Task records.
...
allocated identifiers keep the `TK-###` form while every historical `TK-###`
string in a completed Spec is byte-identical before and after.
