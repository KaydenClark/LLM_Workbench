...
| TK-003 | Prove fresh-session Claude and Codex skill discovery (author -> catalog -> fresh-session discovery) | blocked | Fresh Claude skill-discovery proof is absent | Claude authentication is live; the earlier non-persistent probe supplied no prompt and did not test discovery |
...
| TK-010 | Prepare downstream distribution and the harness upgrade path for the rewritten skills | blocked | TK-003 | pending |
...
| 2026-07-20 | TK-003 slice split (plan-only) | Applied `/tracer-bullet` + `/to-tickets`: the bundled TK-003 ("verify fresh sessions AND prepare downstream distribution") mixed two independently shippable outcomes, so it was split into dependency-ordered tracer bullets. TK-003 now owns fresh-session Claude/Codex discovery proof only (author -> catalog -> fresh-session discovery); new TK-010 owns downstream distribution and the harness upgrade path and is blocked on TK-003. No completed row or prior evidence was altered. The grilling-family contract flagged on 2026-07-17 is promoted to its own proposed spec S-016. | Plan-only edit to the S-011 slice table plus this row; `node tools/spec-workbench.mjs render` and `doctor` passed; hot board still surfaces TK-003 as the current S-011 slice because TK-010 stays blocked behind it | Updated the S-011 slice table and appended this evidence row only; skills `README.md`, RUNBOOK, and BLUEPRINT checked with no update needed because the catalog, lifecycle, and shipped-skill contracts are unchanged by a slice split | Fresh Claude discovery proof still gates TK-003; downstream distribution (TK-010) follows verified discovery; grilling-family ownership moves to S-016 |
