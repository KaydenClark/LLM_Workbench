...
| 2026-07-20 | TK-002 follow-on link (plan-only) | The un-ticketed current-main reconciliation that blocks TK-002 now has a dedicated proposed follow-on spec, S-015 (Main-Into-Integration Reconciliation). S-015/TK-001 owns the audited merge that restores `origin/main` ancestry into `origin/integration` and hands the resulting exact integration SHA back to this ticket for its separate exact-head audit and release-gate publication. This is a linking note only: no branch, merge, PR, status, audit, or existing S-014 evidence row was changed. | Added `specs/S-015-main-integration-reconciliation/SPEC.md` (status `planned`); `node tools/spec-workbench.mjs render` and `doctor` passed with S-015 in the Blueprint catalog and out of the hot board while planned; this row is append-only and alters no prior S-014 evidence | Appended this linking row only; no operational or harness behavior changed, so RUNBOOK, README, templates, and skills need no update | Owner activates S-015 and lands TK-001; then TK-002 audits the reconciled exact integration SHA and TK-003 opens the promotion PR |
