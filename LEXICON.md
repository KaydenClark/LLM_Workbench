...
| **SPEC and TASK** | The Spec is the durable capability owner. Its Tasks divide delivery into temporary slices, each in its own `TASK.md`. Neither a dashboard nor a local task list replaces their accepted state. |
...
| **Ticket** | Retired as a live term. `Task` names the execution slice. | Historical `TK-###` identifiers stay readable exactly as written in append-only evidence and are never rewritten; only newly allocated slices take the Task form. The rename across `AGENTS.md`, `RUNBOOK.md`, the tools, the skills and the generic templates is the open implementation gap owned by [S-00H](workbench/specs/S-00H-task-artifact-and-terminology-migration/SPEC.md), so those still say `Ticket` until its slices land. |
