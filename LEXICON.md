...
| **SPEC and TASK** | The Spec is the durable capability owner. Its Tasks divide delivery into temporary slices, each bound for its own `TASK.md` once [S-00H](workbench/specs/S-00H-task-artifact-and-terminology-migration/SPEC.md) migrates them out of the Spec's slice table. Neither a dashboard nor a local task list replaces their accepted state. |
...
| **Spec** | A stable capability record containing scoped intent, requirements, decisions, its implementation slices, acceptance, verification, evidence, and completion. Accepted Canon moves each slice into its own `TASK.md`; [S-00H](workbench/specs/S-00H-task-artifact-and-terminology-migration/SPEC.md) owns that migration and the slices sit in the Spec's table until it lands. | It combines the useful product and engineering roles often split between a PRD and technical spec. |
| **Task** | The execution slice: one bounded, executable thin vertical slice of a Spec, with its own state and blocking relationships, so independent Tasks can complete in parallel. The Spec and its assigned Task carry the normal stance. | It is execution structure, not durable capability history. Work expected to need more than one context unit is a Spec with Tasks, not a Task ([ADR-000H](workbench/docs/adr/000H-a-task-is-a-standalone-artifact-and-task-replaces-ticket-as-the-execution-slice-term.md)). Accepted Canon gives a Task its own `TASK.md`; until [S-00H](workbench/specs/S-00H-task-artifact-and-terminology-migration/SPEC.md) TK-001 and TK-002 land, a Task is still authored and read as a row in its Spec's slice table, including in Specs written today. |
...
| **Ticket** | Retired as a live term. `Task` names the execution slice. | Historical `TK-###` identifiers stay readable exactly as written in append-only evidence and are never rewritten; only newly allocated slices take the Task form. The standalone `TASK.md` and the rename across `AGENTS.md`, `RUNBOOK.md`, the tools, the skills and the generic templates are both open implementation gaps owned by [S-00H](workbench/specs/S-00H-task-artifact-and-terminology-migration/SPEC.md), so slices stay in Spec tables and those files still say `Ticket` until its slices land. |
