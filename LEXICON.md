...
| **Ticket** | Retired as a live term. `Task` names the execution slice. | Historical `TK-###` identifiers stay readable exactly as written in append-only evidence and are never rewritten; `TK` is the Task identifier prefix (TT-Q10, answered by the owner 2026-09-17), so newly allocated slices keep the `TK-###` form. The rename across `AGENTS.md`, `RUNBOOK.md`, the tools, the skills and the generic templates is an open implementation gap owned by [S-00H](workbench/specs/S-00H-task-artifact-and-terminology-migration/SPEC.md) TK-003 and TK-004, so those files still say `Ticket` until those slices land; the standalone `TASK.md` itself is landed. |
