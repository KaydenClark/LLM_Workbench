...
| **Chat** | One owner-visible working context in a host, in which participants have a Conversation. A Chat works one Task: it completes that Task, cleans up and ends (TT-Q1 and TT-Q4, locked by the owner). | A Chat is not a Task, assignment, durable record, or authority boundary, and it never works several Tasks. Several related Chats may belong to one Thread. |
