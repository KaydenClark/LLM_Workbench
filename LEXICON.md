...
| **Director** | The role above the Dispatchers for a version of work: it starts a Dispatcher for each Spec, coordinates between them, approves each assembled Spec in a separate context before it combines into `integration`, and escalates blockers to the owner. | Defined by responsibility, not by branch. The owner is the human above the Director, not the Director; Human QA stays the owner's own act, normally after the Director has approved the version's Specs or escalated their blockers, at times the owner chooses, with approval recorded per Spec. A Director never executes a Task. It is not a stance or an authority grant, and it never bypasses the `AGENTS.md` integration gate. The [`carry`](workbench/skills/carry/SKILL.md) skill does not describe these roles yet. |
