...
| **Spec** | A stable capability record containing scoped intent, requirements, decisions, its implementation slices, acceptance, verification, evidence, and completion. Accepted Canon moves each slice into its own `TASK.md`; S-00H owns that migration and the slices sit in the Spec's table until it lands. | It combines the useful product and engineering roles often split between a PRD and technical spec. |
